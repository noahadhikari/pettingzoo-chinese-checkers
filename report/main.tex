...
\usepackage{array}
\usepackage{listings}
\usepackage[noline,linesnumbered,ruled]{algorithm2e}
\usepackage{amsmath}
...
We evaluate the performance of the trained policies by having them play against a random agent. The number of wins is recorded, as well as the number of turns taken to win, both of which are used as evaluation metrics. The evaluation procedure is summarized below:
...
\begin{algorithm}
  ...
  \Input{n: Number of evaluation games \\
         policy: Policy configuration}
  ...
  \Return{Number of wins, average number of turns taken}

  \Indm
  \textbf{end}
  \caption{Agent evaluation procedure}
